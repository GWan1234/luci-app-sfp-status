...
function renderStyle() {
	return E('style', [
		...
		'.mb-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin:.75rem 0 1.25rem}',
		'.mb-panel{border:1px solid var(--border-color,#ddd);border-radius:8px;padding:14px;background:var(--background-color-high,#fff)}',
		'.mb-panel h3{margin:0 0 10px;font-size:1.05rem}',
		'.mb-row{display:grid;grid-template-columns:minmax(110px,38%) 1fr;gap:10px;padding:7px 0;border-top:1px solid var(--border-color-low,#eee)}',
		'.mb-row:first-of-type{border-top:0}',
		'.mb-label{color:var(--text-muted,#666)}',
...
function renderSummary(json) {
	return E('div', { 'class': 'cbi-section' }, [
		...
		E('div', { 'class': 'mb-summary' }, BAND_TECHS.map(function(tech) {
